...
async def recipeGenerator(recipe, image_data=None):
...
async def select_main_image(image_data):
    print("👽 wybieram zdjęcie główne przepisu")
    try:
        messages = [
            {
                "role": "system",
                "content": "Jesteś ekspertem kulinarnym, który wybiera najlepsze zdjęcie jako zdjęcie główne przepisu."
            },
            {
                "role": "user",
                "content": f"Na podstawie poniższych danych zdjęć, proszę wybrać najlepsze zdjęcie jako zdjęcie główne przepisu. Pamiętaj, że zdjęcie musi być wysokiej rodzielczośći.\n\n{json.dumps(image_data)}"
            }
        ]
        ...
        response_message = response.choices.message.content.strip()
        print(response_message)
        return response_message
    except Exception as e:
        print(f"Wystąpił błąd: {e}")
